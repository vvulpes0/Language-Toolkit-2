...
>                          FSA n1 e
>                       -> FSA n2 e
>                       -> FSA (Maybe (n1, Maybe Integer)) e
> autStrictOrderOverlay f1 f2
>     = out { transitions = transitions out `Set.union` newInitTrs
>           , initials = Set.singleton (State Nothing)
>           , finals = if accepts f1 [] && accepts f2 []
>                      then Set.insert (State Nothing) (finals out)
>                      else finals out
>           , isDeterministic = False
>           }
>     where alpha = alphabet f1 `Set.union` alphabet f2
>           tx = totalWithAlphabet alpha
>           _ = states tx :: Set (State ()) -- to fix the type
>           f1' = (renameStatesBy (Just . flip (,) Nothing) f1)
>                 { sigma = alpha, finals = Set.empty }
>           out = Set.foldr g f1' (states f1)
>           f q = renameStatesBy (Just . ((,) (nodeLabel q)) . Just)
>                 . normalize $ autIntersection f2
>                 (autConcatenation
>                  (f1 { initials = Set.singleton q }) tx)
>           qi = epsilonClosure f1' $ initials f1'
>           newInitTrs = Set.map (\t -> t { source = State Nothing })
>                        . Set.filter (isIn qi . source)
>                        $ transitions f1'
>           g x a = let x' = f x
>                       n = nodeLabel x
>                       t = Set.map (\q ->
>                                    Transition
>                                    { edgeLabel = Epsilon
>                                    , source = State (Just (n, Nothing))
>                                    , destination = q
>                                    }
>                                   ) (initials x')
>                   in a { transitions
>                              = Set.unions
>                                [ t, transitions a, transitions x']
>                        , finals = finals a `Set.union` finals x'
>                        }
