...
{- Forbidden Factors
   ...
   Final FF are labels of paths Q --> state sets other than $\emptyset$ that are 
...
> forbiddenFactorsWithAlphabet fsa alph = (uFFs, fWs, iFFs, frFFs, fiFFs)
>     where
...
> forbiddenFactors fsa = forbiddenFactorsWithAlphabet fsa defaultAlphabet
>                        
> {- Forbidden Units
...
>             (\wrd -> not ((null wrd)
>                           ||
>                           (any (\x -> (List.isSuffixOf x wrd)) (Set.toList fiFFs))
>                           ||
...
>                    - 2
>                  supermax :: Set.Set [a] -> Int
>                  supermax l
>                      | (Set.null l) = 0
>                      | otherwise = (maximum . map length) (Set.toList l)
> 
> 
...
> 
> freeFFs :: (Ord a, Ord b) => (FSA b a) -> Int -> Set.Set [Symbol a]
> freeFFs fsa k = freeFFsPSG (generatePowerSetGraph fsa) k
> 
> -- k is only significant if it is 0
> freeFFsPSG :: (Ord a, Ord b) => (FSA (Set.Set b) a) -> Int ->
>                                  Set.Set [Symbol a]
> freeFFsPSG psg k
...
>     where
>       (nextFront,nextFFs)  -- (front k+1, k-FFs)
>           = passK goal                   
...
>       was = map (renameStates . minimize . complement .
>                                   (singIntWithAlphabet alphs))
>                 ([]:(Set.toList fws)) -- forbid epsilon
>       ias =  map
>              (renameStates . minimize . complement .
>                                (initialfactorWithAlphabet alphs))
>              (Set.toList ifs)
>       fras =  map (renameStates . minimize . complement .
>                                     (factorWithAlphabet alphs))
>               (Set.toList frfs)
>       fias =  map (renameStates . minimize . complement . 
>                                     (finalfactorWithAlphabet alphs))
>                   (Set.toList fifs)
> 
> -- build FSA from FSAs
...
>     renameStates (foldr flatIntersection (allWithAlphabet alphs)
>                             (foldr (++) [] [was,ias,fras,fias]) )
> 
> 
